...
df <- fread("C:/Users/terla/Desktop/R/QSS/session4/Future-500.csv",na.strings = "")
...
df %>% View() # 15 ve 16ci setirler silindi
...
df[which(df[["Revenue"]] %in% o3),"Revenue"] <- val

val <- quantile(df[["Revenue"]],0.25) - 1.5*IQR(df[["Revenue"]])
df[which(df[["Revenue"]] %in% o1),"Revenue"] <- val
...
df[which(df[["Employees"]] %in% o3),"Employees"] <- val

val <- quantile(df[["Employees"]],0.25) - 1.5*IQR(df[["Employees"]])
df[which(df[["Employees"]] %in% o1),"Employees"] <- val
...
df[which(df[["Profit"]] %in% o3),"Profit"] <- val

val <- quantile(df[["Profit"]],0.25) - 1.5*IQR(df[["Profit"]])
df[which(df[["Profit"]] %in% o1),"Profit"] <- val
